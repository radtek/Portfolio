...
function CreateEmbeddedMovie(e, url)
{
	...
	var sExt   = url.match(/\.(mpg|mpeg|avi|wmv|mov|asf|rm|rmvb)$/i);	

	if (sExt.length && sExt.length > 0)
		sExt = sExt[0];
	else
		sExt = '';
	sExt = sExt.substring(1,sExt.length);
	sType = (sExt=="mpg"||sExt=="mpeg")             ?"video/mpeg":
          (sExt=="avi"||sExt=="wmv"||sExt=="asf") ?"video/x-msvideo":
				  (sExt=="mov") ?"video/quicktime" : 
            		(sExt=="rm"||sExt=="rmvb") ?"audio/x-pn-realaudio-plugin" : "video/x-msvideo";
	...
	{
		if (sExt=="rm"||sExt=="rmvb")
		{
			//html = '<embed SRC type="audio/x-pn-realaudio-plugin" CONSOLE="Clip1" CONTROLS="ImageWindow" HEIGHT="288" WIDTH="352" AUTOSTART="false">
			html = '<EMBED type="audio/x-pn-realaudio-plugin" src="'+ url +'" '+				   
			...
			/*html += 'controls="ImageWindow,playbutton,pausecutton,stopbutton';
			if (!GetE('chkShowcontrols').checked)
			{				
				if (!GetE('chkShowaudiocontrols').checked)
					html += ',volumelever';				
			}
			if (!GetE('chkShowstatusbar').checked)
					html += ',statusbar';
			html += '"';*/
			html += 'controls="smallconsole"';
...
{	
	...
	var sExt   = GetE('txtUrl').value.match(/\.(mpg|mpeg|avi|wmv|mov|asf|rm|rmvb)$/i);	
	if (!sExt)
	{
		window.parent.SetSelectedTab( 'Info' ) ;
		GetE('txtUrl').focus() ;

		alert( FCKLang.EmbedMoviesFileType ) ;
