...
namespace Johnny.CMS.DAL.SystemInfo
{
    ...
    public class Navigator
    {
        ...
        {
            IList<Johnny.CMS.OM.SystemInfo.Navigator> list = new List<Johnny.CMS.OM.SystemInfo.Navigator>();
            ...
            using (SqlDataReader sdr = DbHelperSQL.ExecuteReader(strSql.ToString()))
            {
                while (sdr.Read())
                {
                    Johnny.CMS.OM.SystemInfo.Navigator item = new Johnny.CMS.OM.SystemInfo.Navigator(sdr.GetInt32(0), sdr.GetString(1), sdr.GetString(2), sdr.GetInt32(3));
                    list.Add(item);
                }
            }
            return list;
        ...
        public Johnny.CMS.OM.SystemInfo.Navigator GetModel(int navigatorid)
        {
            ...
            Johnny.CMS.OM.SystemInfo.Navigator model = null;
            ...
            {
                if (sdr.Read())
                    model = new Johnny.CMS.OM.SystemInfo.Navigator(sdr.GetInt32(0), sdr.GetString(1), sdr.GetString(2), sdr.GetInt32(3));
                else
                    model = new Johnny.CMS.OM.SystemInfo.Navigator();
            }
            return model;
        }
