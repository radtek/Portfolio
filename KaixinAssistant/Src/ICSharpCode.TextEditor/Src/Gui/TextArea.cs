...
//				clipRectangle.Intersect(updateMargin.DrawingPosition);
		...
		internal bool IsReadOnly(int offset)
		{
			...
			if (TextEditorProperties.SupportReadOnlySegments) {
                ...
                return Document.MarkerStrategy.GetMarkers(offset).Exists(delegate(TextMarker m) { return m.IsReadOnly; });
			} else {
				return false;
			}
		}
		...
		internal bool IsReadOnly(int offset, int length)
		{
			...
			if (TextEditorProperties.SupportReadOnlySegments) {
                ...
                return Document.MarkerStrategy.GetMarkers(offset, length).Exists(delegate(TextMarker m) { return m.IsReadOnly; });
			} else {
				return false;
			}
		}
